...
pkgdatadir = $(datadir)/libsigsegv
pkglibdir = $(libdir)/libsigsegv
pkgincludedir = $(includedir)/libsigsegv
...
STRIP = strip
VERSION = 2.6
abs_builddir = U:/progetti/libsigsegv-2.6
abs_srcdir = U:/progetti/libsigsegv-2.6
abs_top_builddir = U:/progetti/libsigsegv-2.6
abs_top_srcdir = U:/progetti/libsigsegv-2.6
ac_ct_CC = gcc
ac_ct_DUMPBIN = 
am__include = include
am__leading_dot = .
...
check-next:
	@when="Now"; \
	if grep '^i386-pc-os2-emx .* 2.6$$' $(srcdir)/PORTING > /dev/null; then :; else \
	  case 'i386-pc-os2-emx' in \
	    ...
	    *) \
	      ...
	      echo "successfully tested platforms (see PORTING file).  Please also"; \
	      echo "send the config.log file; this will help improving portability"; \
	      echo "of the package."; echo; \
	      if test 0 = 1; then have1=yes; else have1=no; fi; \
	      if test 0 = 1; then have2=yes; else have2=no; fi; \
	      echo `cat termbold`"libsigsegv: i386-pc-os2-emx | $$have1 | $$have2 | 2.6"`cat termnorm`; \
	      when="Then"; \
	      ;; \
	  esac; \
	fi; \
